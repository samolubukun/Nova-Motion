...
### 14. Stickman Explainer Short (`videoType: "StickmanExplainerVideo"`)
A Stickman-Studio-style educational explainer generator — one topic in, a narrated stickman animation out. The cheapest mode in the suite: slideshow mode costs ~$0.05/video.
* **Viral storyboard**: The WaveSpeed LLM turns any topic into a fast-paced scene plan following viral Shorts rules — scene 1 is always a hook (shocking fact / weird question / extreme visual), narration stays punchy and conversational, and the stickman performs exaggerated physical comedy.
* **One canonical character reference**: A single T-pose stickman reference sheet (Seedream) anchors the whole video's identity.
* **Reference-locked scenes**: Every scene image is generated by Seedream v4 Edit with the character reference attached as a real image input ($0.027/image; falls back to text-to-image; override via `STICKMAN_IMAGE_EDIT_MODEL`).
* **Slideshow mode (default, near-free)**: Scene stills are animated natively in Remotion with an alternating Ken Burns zoom/pan (the reference project's ffmpeg `zoompan`, re-implemented as spring math) — zero video-generation cost.
* **Animated mode**: Each scene becomes a Wan 2.2 I2V clip ($0.10/5s; override via `STICKMAN_I2V_MODEL`) that plays the scene's action with cartoon motion while preserving the flat line-art world.
* **Narration & captions**: Per-scene narration voiced by ElevenLabs (preferred) / Deepgram TTS with word-level timestamps, rendered as lower-third white-on-black-stroke captions; optional Lyria background music.
* **Async**: Returns a `jobId` immediately. Poll `GET /api/videos/{jobId}` for render status.

```json
{
  "videoType": "StickmanExplainerVideo",
  "prompt": "How black holes work",
...
* **`videoType`** (string, required): The video template mode. Must be one of:
  ...
  * `"StickmanExplainerVideo"`: Stickman-Studio-style educational short — hook storyboard → stickman character reference → reference-locked scene images → Ken Burns slideshow or animated I2V clips + narrator TTS & captions.
...
#### 16. Stickman Explainer Short (`StickmanExplainerVideo`)
Replicates the Stickman Studio pipeline entirely on the WaveSpeed stack. A topic flows through: **LLM storyboard** (viral hook rules — scene 1 shocks, punchy conversational narration, exaggerated stickman physical comedy) → one **Seedream T-pose character reference** → per scene a **reference-locked scene image** (Seedream v4 Edit with the char ref attached; falls back to text-to-image) → animation via either **slideshow mode** (free alternating Ken Burns zoom/pan rendered natively in Remotion) or **animated mode** (one Wan 2.2 I2V clip per scene) → per-scene **narrator TTS** with word timestamps → optional **Lyria** music → assembled by the `StickmanExplainerVideo` composition with scene chips, lower-third captions and a marker-style title card.
...
* **Fields**: `sceneCount` = number of scenes (2-10; scene 1 doubles as the hook). `animation`: `slideshow` (Ken Burns zoom on stills, ~$0.05/video total) or `animated` (Wan I2V clip per scene, ~$0.70/video). `prompt` accepts a topic, question, or raw explainer script.
* **Requires** `WAVESPEED_API_KEY` (char ref + scene images + animated-mode clips + music) and a TTS key (`ELEVENLABS_API_KEY` preferred or `DEEPGRAM_API_KEY`). The storyboard LLM uses `STICKMAN_LLM_URL`/`STICKMAN_LLM_MODEL` (falling back to the ComicDrama/ZackD LLM vars); scene images are reference-locked via `STICKMAN_IMAGE_EDIT_MODEL` (default `bytedance/seedream-v4/edit`); set `STICKMAN_I2V_MODEL` to change the animated-mode clip model.
* **Async** — returns a `jobId` immediately; poll `GET /api/videos/{jobId}` until completed.
