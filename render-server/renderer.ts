...
if (job.videoType === "AgenticVideoGenerator" && job.pipeline) {
    ...
    if (job.videoType === "StickmanExplainerVideo" && job.pipeline) {
      console.log(`[StickmanExplainerVideo] Running stickman-explainer pipeline for job ${job.id}...`);
      const pipeline = job.pipeline;
      const input: StickmanExplainerVideoInput = {
        prompt: pipeline.prompt || "",
        title: pipeline.title,
        targetDurationSeconds: pipeline.targetDurationSeconds,
        language: pipeline.language,
      ...
      job.timeline = await generateStickmanExplainerTimeline(input, {
        jobId: job.id,
        assetBaseUrl: baseUrl,
        onProgress: (progress) => updateJobStatus(job.id, { progress: Math.round(progress * 25) }),
        onStage: (stage: string) => updateJobStatus(job.id, { currentStage: stage }),
