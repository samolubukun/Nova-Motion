...
import { AgenticVideoRequestSchema, RenderRequestSchema, TextToVideoRequestSchema, MicroDramaRequestSchema, UGCRequestSchema, LumaRequestSchema, VoxRequestSchema, ZackDRequestSchema, ComicDramaRequestSchema, StickmanExplainerRequestSchema, WhiteboardVideoRequestSchema } from "../shared/video-schema";
...
app.post("/render/luma", authenticate, (req: Request, res: Response) => {
  try {
    const validation = LumaRequestSchema.safeParse(req.body);
    if (!validation.success) {
      return res.status(400).json({
        error: "Invalid request",
        details: validation.error.issues.map((e) => ({
    ...
    console.log(`Luma job created: ${job.id}`);
    res.status(201).json({
      jobId: job.id,
      status: job.status,
      createdAt: job.createdAt.toISOString(),
...
app.post("/render/comic-drama", authenticate, (req: Request, res: Response) => {
  try {
    const validation = ComicDramaRequestSchema.safeParse(req.body);
    if (!validation.success) {
      return res.status(400).json({
        error: "Invalid request",
        details: validation.error.issues.map((e) => ({
    ...
    console.log(`ComicDramaVideo job created: ${job.id}`);
    res.status(201).json({
      jobId: job.id,
      status: job.status,
      createdAt: job.createdAt.toISOString(),
    ...
    console.error("Error creating ComicDramaVideo job:", err);
    res.status(500).json({
      error: err instanceof Error ? err.message : "Internal server error",
    });
  }
...
app.post("/render/stickman-explainer", authenticate, (req: Request, res: Response) => {
  try {
    const validation = StickmanExplainerRequestSchema.safeParse(req.body);
    if (!validation.success) {
      return res.status(400).json({
        error: "Invalid request",
        details: validation.error.issues.map((e) => ({
    ...
    console.log(`StickmanExplainerVideo job created: ${job.id}`);
    res.status(201).json({
      jobId: job.id,
      status: job.status,
      createdAt: job.createdAt.toISOString(),
    ...
    console.error("Error creating StickmanExplainerVideo job:", err);
    res.status(500).json({
      error: err instanceof Error ? err.message : "Internal server error",
    });
  }
...
app.post("/render/whiteboard", authenticate, (req: Request, res: Response) => {
  try {
    const validation = WhiteboardVideoRequestSchema.safeParse(req.body);
    if (!validation.success) {
      return res.status(400).json({
        error: "Invalid request",
        details: validation.error.issues.map((e) => ({
    ...
    console.log(`WhiteboardVideo job created: ${job.id}`);
    res.status(201).json({
      jobId: job.id,
      status: job.status,
      createdAt: job.createdAt.toISOString(),
    ...
    console.error("Error creating WhiteboardVideo job:", err);
    res.status(500).json({
      error: err instanceof Error ? err.message : "Internal server error",
    });
  }
...
Endpoints:
  ...
  POST /render/agentic-video - Submit an end-to-end agentic video job
   POST /render/luma        - Submit a Luma Ray 3.2 (all use-cases) video job
    ...
    POST /render/comic-drama - Submit an AI comic / anime drama episode job
    POST /render/stickman-explainer - Submit a stickman educational explainer job
